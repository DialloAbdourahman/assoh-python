...
class EnumOrderStatus(Enum):
    ...
    EXPIRED='EXPIRED' # The user did not pay and stripe sent us an event for that, we free the products and nothing to be done on the financial line. 
...
# Cancel the order completely if there is a timeout. 
# Come back to the cancel order after payment and complete the whole process (refunds and everything)
# Comeback and handle the stripe payment error and allow the user to retry failed payments. 
